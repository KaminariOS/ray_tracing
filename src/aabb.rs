        ...
        match (bbox0, bbox1) {
            (Some(b0), Some(b1)) => {
                ...
                b0.minimum
                    ...
                    .zip(b1.minimum.iter())
                    .enumerate()
                    .for_each(|(i, (min0, min1))| small[i] = min0.min(*min1));
                let mut big = [0.; 3];
                b0.maximum
                    .iter()
                    .zip(b1.maximum.iter())
                    .enumerate()
                    .for_each(|(i, (max0, max1))| big[i] = max0.max(*max1));
...
impl BVHNode {
    pub fn new(objects: &[SharedHittable], time0: f32, time1: f32, label: Option<String>) -> Shared<Self> {
        ...
        let (left, right) = if obj_span == 1 {
            ...
            objects.sort_by(|x, y| {
                Self::box_compare(x, axis)
                    .partial_cmp(&Self::box_compare(y, axis))
                    .expect("NaN")
            });
            let mid = obj_span / 2;
            (
